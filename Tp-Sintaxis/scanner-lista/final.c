...
int main(int argc, char * argv[]) {
    ...
    int l; /************************Se abre el Archivo Fuente******************/
    if (argc == 1) {
...
void ListaIdentificadores(void) { /* <listaIdentificadores> -> <identificador> #leer_id {COMA <identificador> #leer_id} */
  TOKEN t;
  REG_EXPRESION reg;
  Identificador( & reg);
  Leer(reg);
...
void Primaria(REG_EXPRESION * presul) {
  ...
  case CONSTANTE:
    ...
    break;
  case PARENIZQUIERDO:
    /* <primaria> -> PARENIZQUIERDO <expresion> PARENDERECHO */ Match(PARENIZQUIERDO);
    Expresion(presul);
    Match(PARENDERECHO);
    break;
  default:
...
int Buscar(char * id, RegTS * TS, TOKEN * t) {
  ...
  while (strcmp("$", TS[i].identifi)) {
    if (!strcmp(id, TS[i].identifi)) { * t = TS[i].t;
      return 1;
    }
    i++;
  }
